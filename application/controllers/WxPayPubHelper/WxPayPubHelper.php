...
class Common_util_pub
{
    ...
    /**
     *     作用：使用证书（包括rootca），以post方式提交xml到对应的接口url
     */
    function postXmlSSLCurl2($xml,$url,$second=30)
    {
        $ch = curl_init();
        //超时时间
        curl_setopt($ch,CURLOPT_TIMEOUT,$second);
...
class Wxpay_client_pub extends Common_util_pub 
{
     ...
     *     作用：使用证书post请求xml
     */
    function postXmlSSL2()
    {    
        $xml = $this->createXml();
        $this->response = $this->postXmlSSLCurl2($xml,$this->url,$this->curl_timeout);
        return $this->response;
    }
    ...
    function createXml()
    {
        try
        {
            ...
            if($this->parameters["partner_trade_no"] == null){
                throw new SDKRuntimeException("缺少转账接口必填参数partner_trade_no！"."<br>");
            }elseif($this->parameters["check_name"] == null){
                throw new SDKRuntimeException("缺少转账接口必填参数check_name！"."<br>");
            }elseif ($this->parameters["re_user_name"] == null ) {
                throw new SDKRuntimeException("缺少转账接口必填参数re_user_name！"."<br>");
            }elseif ($this->parameters["amount"] == null) {
                throw new SDKRuntimeException("缺少转账接口必填参数amount！"."<br>");
            }elseif($this->parameters["desc"] == null){
                throw new SDKRuntimeException("缺少转账接口必填参数desc！"."<br>");
            }
            ...
            $this->parameters["spbill_create_ip"] = $_SERVER['REMOTE_ADDR'];//终端ip
            $this->parameters["nonce_str"] = $this->createNoncestr();//随机字符串
            $this->parameters["openid"] = $this->openid;//用户openid
            $this->parameters["sign"] = $this->getSign($this->parameters);//签名
