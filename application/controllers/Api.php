	...
	public function land($com = 'type', $sub = 'list') {
		if ($com == 'type') {
			...
			foreach($lands as $land){
				...
				$data[] = array($land->id, $land->map, $land->landnum, $types[$land->type], $blocks[$land->block], $detail, $user, ($land->sold_at)?date( 'Y年m月d日', strtotime($land->sold_at)):'', $status, null, $land->status);
			}
			...
			if ($this->input->post('change_map') === '1') {
				unlink($land['map']);
				$picture = str_replace(array('data:image/png;base64,', ' '), array('', '+'), $this->input->post('image'));
			...
			if ($this->input->post('change_intro') === '1') {
				unlink($land['intro']);
				$path = './uploads/lands/';
			...
			$this->Land_model->update_land($id, $params);
	...
	public function upload($com = '', $sub = '') {
		...
		if (strlen($total) > 0) {
			...
			if ($total == 'product') {
				...
				for ($i = 1, $len = count($pictures); $i < $len; $i++) {
					$picture = str_replace(array('data:image/png;base64,', ' '), array('', '+'), $pictures[$i]);
					$data = base64_decode($picture);
					$file = 'uploads/products/0_' . md5($picture) . '.png';
					file_put_contents($file, $data);
					$params = array(
					...
					$this->Product_detail_img_model->add_product_detail_img($params);
				}
