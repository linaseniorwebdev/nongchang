let table, curID, curValue, costValue;
		...
		columnDefs: [
			...
			},
			{
				...
				render: function ( data, type, row ) {
					...
					buffer += ('<input type="hidden" value="' + row[3] + '" />');
					return buffer;
				},
				orderable: false
			}
...
function change(obj) {
	...
	costValue = obj.nextElementSibling.nextElementSibling.value;
	costValue = costValue.substring(1);
...
function confirm() {
	...
	costValue = parseFloat($("#costInput").val());
	if (isNaN(costValue)) {
		swal("警告", "请检查输入是否正确", "warning");
		return;
	}
	if ($("#cat_edit").val() < 0) {
		swal("警告", "请选择类别", "warning");
		return;
	}
